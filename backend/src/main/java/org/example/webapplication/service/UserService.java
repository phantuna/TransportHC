...
public class UserService {
    ...
    public List<UserResponse> getAvailableDrivers(String truckId) {
        permissionService.getUser(
                List.of(PermissionKey.VIEW),
                PermissionType.REPORT
        );
        List<User> drivers = userRepository.findAvailableDrivers(truckId);
        return drivers.stream().map(userMapper::toResponse).toList();
    }
}
