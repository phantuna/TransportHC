...
public class InventoryRepositoryCustomImpl implements InventoryRepositoryCustom {
    ...
    private BooleanBuilder buildWhere(
        ...
        if(itemId != null && !itemId.isBlank()) {
            builder.and(qInventory.item.id.eq(itemId));
        }

        if(status != null) {
            builder.and(qInventory.status.eq(status));
        }

        if(keyword != null && !keyword.isBlank()) {
            builder.and(qInventory.description.containsIgnoreCase(keyword));
        }

        if(fromDate != null) {
            builder.and(qInventory.createdDate.goe(LocalDate.from(fromDate.atStartOfDay())));
        }

        if(toDate != null) {
            builder.and(qInventory.createdDate.loe(LocalDate.from(toDate.atTime(23,59,59))));
        }
