...
public class PayrollService {
    ...
    @Transactional
    public PageResponse<PayrollDetailResponse> payrollAllByMonth(
            int month,
            int year,
            int page,
            int size
        ...
        List<Payroll> payrolls = payrollRepository.payrollByMonth(month, year, pageable);
        List<PayrollDetailResponse> responses = payrolls.stream()
                .map(this::payrollSheetByMonth)
                .toList();

        long totalElements = payrollRepository.countByMonthAndYear(month, year);
        int totalPages = (int) Math.ceil((double) totalElements / size);

        return PageResponse.<PayrollDetailResponse>builder()
                .content(new ArrayList<>(responses)) // Sử dụng biến responses ở trên
                .page(page)
                .size(size)
                .totalElements(totalElements)
                .totalPages(totalPages > 0 ? totalPages : 1) // Tránh trường hợp chia ra 0 trang
                .build();
    }
